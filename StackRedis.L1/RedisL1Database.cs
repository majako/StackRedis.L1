...
namespace StackRedis.L1
{
    public class RedisL1Database : IDatabase, IDisposable
    {
        ...
        {
            if (_redisDb == null) throw new NotImplementedException();
            return _redisDb.SortedSetRangeAndStore(sourceKey, destinationKey, start, stop, sortedSetOrder, exclude, order, skip, take, flags);
        }
        ...
        {
            if (_redisDb == null) throw new NotImplementedException();
            return _redisDb.StringGetSetExpiry(key, expiry, flags);
        }
        ...
        public RedisValue StringGetDelete(RedisKey key, CommandFlags flags = CommandFlags.None)
        {
            ...
            if (isInMemory)
            {
                _dbData.MemoryCache.Remove(new[] { key.ToString() });
            }
            return result;
        }
        ...
        {
            _dbData.MemoryCache.Add(key, value, expiry, when);
            return _redisDb == null || _redisDb.StringSet(key, value, expiry, keepTtl, when, flags);
        }
        ...
        {
            if (_redisDb == null) throw new NotImplementedException();
            return _redisDb.SortedSetRangeAndStoreAsync(sourceKey, destinationKey, start, stop, sortedSetOrder, exclude, order, skip, take, flags);
        }
        ...
        public async Task<RedisValue> StringGetDeleteAsync(RedisKey key, CommandFlags flags = CommandFlags.None)
        {
            ...
            var isInMemory = true;
            var result = await _dbData.MemoryStrings.GetFromMemoryAsync(key, async () =>
            {
                isInMemory = false;
                return await _redisDb.StringGetDeleteAsync(key, flags).ConfigureAwait(false);
            });
